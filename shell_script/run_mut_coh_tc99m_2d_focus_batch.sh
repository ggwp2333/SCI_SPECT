...
CUBE_DIR="crystal_geometry_rand_design_v2"
MAP_DIR="crystal_geometry_rand_design_v2"
OUT_DIR="/datastore01/user-storage/huitian/SCI_SPECT/mutual_coherence"
ALL_MUT_COH_BIN="voxel_crosstalk/voxel_crosstalk_rand_v2.bin"
...
run_worker() {
    local worker_id=$1
    local dev=$worker_id
    local i tag cube map mut_coh_tmp
    ...
    for (( i=0; i<N; ++i )); do
        ...
        mut_coh_tmp="${OUT_DIR}/correlation_${tag}.bin"

        echo "[GPU $dev] Processing tag '$tag'..."
        ./executables/vc_tc99m_120mmFOV "$cube" "$map" "$mut_coh_tmp" "$dev"
    done
}
...
for (( d=0; d<N_DEVICES; ++d )); do
    ...
    for ((i=0; i<N; ++i)); do
        ...
        if [ ! -f "$mut_coh_tmp" ]; then
            echo "WARNING: missing fwhm file for tag '$tag' -> $mut_coh_tmp" >&2
            continue
        fi

        cat "$mut_coh_tmp" >> "$ALL_MUT_COH_BIN"
    done
    ...
    echo "Done. Aggregated correlation values in: $ALL_MUT_COH_BIN"
