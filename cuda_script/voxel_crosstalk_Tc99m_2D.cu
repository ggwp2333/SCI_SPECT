...
__global__ void sen_map_kernel(const double* __restrict__ d_H, int row_count, int col_count, double* __restrict__ d_sen_map)
{
    ...
    for (int col = 0; col < col_count; ++col) {
        s += d_H[row + (size_t)col * row_count];
    }
    d_sen_map[row] = s;
}

__global__ void inv_kernel(const double* __restrict__ d_sen_map, int row_count, double* __restrict__ d_w)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= row_count) return;
    double s = d_sen_map[i];
    d_w[i] = (s > 0.0) ? (1.0 / s) : 0.0;
}

__global__ void extract_diag_kernel(const double* __restrict__ d_G, int col_count, double* __restrict__ d_diagG)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= col_count) return;
    d_diagG[i] = d_G[i + (size_t)i * col_count];
}

__global__ void ens_crosstalk_kernel(const double* __restrict__ d_G, int col_count, const double* __restrict__ d_diagG, double* __restrict__ d_crosstalk)
{
    int j = blockIdx.x * blockDim.x + threadIdx.x;
    int i = blockIdx.y * blockDim.y + threadIdx.y;
    if (i >= col_count || j >= col_count || i >= j) return;

    double di = d_diagG[i], dj = d_diagG[j];
    if (di <= 0.0 || dj <= 0.0) return;

    double gij = d_G[i + (size_t)j * (size_t)col_count];
    double term = (gij * gij) / (di * dj);
    atomicAdd(d_crosstalk, 2.0 * term);
}
                   ...
                   double& h_loss)
{
    ...
    int* d_map; double3* d_cubes; double* d_H; double* d_f; double* d_g; double* d_w; double* d_F; double*  d_loss; double* d_sen_map = nullptr; double* d_diagF = nullptr;
    ...
    CUDA_CHECK(cudaMalloc(&d_sen_map, row_count*sizeof(double)));
    CUDA_CHECK(cudaMalloc(&d_diagF, col_count*sizeof(double)));
    ...
    {
        int threads = 256;
        int blocks  = (col_count + threads - 1) / threads;
        extract_diag_kernel<<<blocks, threads>>>(d_F, col_count, d_diagF);
        CUDA_CHECK(cudaGetLastError());
    }
    
    {
        dim3 block(16, 16);
        dim3 grid((col_count + block.x - 1) / block.x, (col_count + block.y - 1) / block.y);
        ens_crosstalk_kernel<<<grid, block>>>(d_F, col_count, d_diagF, d_loss);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
