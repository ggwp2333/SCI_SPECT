                   ...
                   std::vector<double>& F)
{
    ...
    int* d_map; double3* d_cubes; double* d_H; double* d_f; double* d_g; double* d_w; double* d_F;
    ...
    // x = 40 * ones(N)
    {
        ...
        create_uniform_phantom<<<b, t>>>(d_f, 40.0);
        CUDA_CHECK(cudaGetLastError());
    }
    ...
    // w = 1./y (elementwise)
    {
        ...
        inv_proj<<<b, t>>>(d_g, d_w, (int)row_count);
        CUDA_CHECK(cudaGetLastError());
    }
    ...
    // Scale rows of H in-place: H(i,j) *= sqrt(w[i])
    {
        ...
        scale_sysmat<<<b, t>>>(d_H, d_w, (int)row_count, (int)col_count);
        CUDA_CHECK(cudaGetLastError());
    }
    ...
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaMemcpy(F.data(),d_F,col_count*col_count*sizeof(double),cudaMemcpyDeviceToHost));
...
double compute_avg_fim_fwhm(const std::vector<double>& F)
{
    ...
    for (int k = 0; k < NSUP; ++k) {
        int vox_idx = sup_lin[k];   // column index in F (0-based)
        int x_ind   = sup_ix[k];    
        int y_ind   = sup_iy[k];    
...
int main(int argc, char** argv){
    try{
        ...
        // ---------------- Query GPUs ----------------
        // std::cout<<"Total elements of the system matrix: "<<Ncubes_bottom*NANG*NVOX<<".\n";
        std::vector<double> F((size_t)NVOX*NVOX,0.0);

        // Launch GPU Computation
        int v_start = 0;
        int v_count = NVOX;
        run_gpu_chunk(dev, v_start, v_count, Ncubes_bottom, h_map, h_bottom_cubes, F);

        // Compute FWHM
        double avg_fim_fwhm = compute_avg_fim_fwhm(F);
        std::cout << "Average FIM FWHM = " << avg_fim_fwhm << std::endl;

        std::ofstream out(out_path,std::ios::binary);
        out.write(reinterpret_cast<const char*>(&avg_fim_fwhm), (std::streamsize)(sizeof(double)));
        out.close();

        /*std::ofstream out(out_path,std::ios::binary);
        out.write(reinterpret_cast<const char*>(F.data()), (std::streamsize)(F.size()*sizeof(double)));
        out.close();*/
    } 
