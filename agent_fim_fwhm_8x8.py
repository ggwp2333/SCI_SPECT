...
from utils import compute_reward, masked_softmax, save_history
...
class DummyRLAgent:
    ...
    def sample_sub_block(self) -> np.ndarray:
        ...
        sub_block = np.ones((4, 4, 6), dtype=np.int8)
        flat_indices = np.arange(96)

        # choose 38 non-crystal positions (~60% crystal occupancy)
        non_crystal_idx = self.rng.choice(flat_indices, size=38, replace=False)
        sub_block_flat = sub_block.reshape(-1)
        sub_block_flat[non_crystal_idx] = 0

        return sub_block.reshape((4, 4, 6))
...
class RLAgent:
    def __init__(
        ...
        self.net = ActorCriticNet(n_actions=96, max_steps=38).to(self.device)
    ...
    def run_episode(self, batch_size: int) -> dict:
        ...
        Runs a single episode:
          - start with empty action_map (4x4x6 = 96 actions)
          - for t in 0..37: select 38 distinct voxels as non-crystal
        ...
        # state init
        action_map = torch.zeros((B, 4, 4, 6), device=self.device)
        ...
        for t in range(T):
            ...
            # C-order strides for (4, 4, 6): (24, 6, 1)
            z = action // 24
            rem = action % 24
            y = rem // 6
            x = rem % 6
        ...
        # Build sub_block: 1 = crystal, 4 = acrylic (non-crystal fill)
        crystal_mask = (1.0 - action_map).detach().cpu().numpy().astype(np.int8).reshape(B, 4, 4, 6)
        sub_block = np.where(crystal_mask == 1, 1, 4)
