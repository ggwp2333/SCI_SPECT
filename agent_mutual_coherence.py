...
from utils import compute_reward, masked_softmax, evaluate_mc_fwhm, save_history
...
class RLAgent:
    ...
    def run_episode(self, batch_size: int) -> dict:
        ...
        k = min(10, metric.size)
        avg_metric = float(np.partition(metric, k - 1)[:k].mean())
    ...
    def train(self, num_episodes: int, batch_size: int = 64, print_every: int = 1, save_every: int = 20):
        ...
        for ep in range(1, num_episodes + 1):
            ...
            if ep % print_every == 0:
                print(
                    f"[Ep {ep:4d}] "
                    f"avg_metric={avg_metric:.2f}  min_metric={min_metric:.2f}  "
                    f"avg_reward={avg_reward:.3f}  best_metric={self.best_metric:.2f}  "
                    f"loss={loss.item():.3f}"
                )
                cmd = f"cp mc_fwhm/mc_fwhm_agent.bin logs/mc_fwhm/ep{ep:03d}.bin"
