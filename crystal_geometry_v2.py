...
class CrystalGeometryTemplate:
    ...
    def _build_full_block(self, sub_block: np.ndarray) -> np.ndarray:
        ...
        """
        sub_block = np.asarray(sub_block, dtype=np.int8).reshape((9, 3, 6), order="F")

        base = sub_block.reshape(-1, order="F")         
        num_sub_block = 3*1*1                     
        full_map = np.tile(base[:, None], (1, num_sub_block)) 

        tmp = full_map.reshape((9, 3, 6, 1, 3, 1), order="F")
...
if __name__ == "__main__":
    ...
    sub_block = np.zeros((9, 3, 6), dtype=np.int8)
    sub_block[0, 0, 0] = 1
    sub_block[1, 1, 1] = 1
    sub_block[2, 2, 2] = 1
    sub_block[0, 0, 3] = 1
    ...
    cube_pos_path = "debug/cube_pos_test.txt"
    np.savetxt(cube_pos_path, cube_pos, fmt="%5.2f")

    map_path = "debug/map_test.txt"
    np.savetxt(map_path, map_reshape, fmt="%2d")
