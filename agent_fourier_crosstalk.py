...
from utils import compute_reward, masked_softmax, evaluate_fourier_crosstalk, save_history
    ...
    def train(self, num_episodes: int, batch_size: int = 64, print_every: int = 1, save_every: int = 20):
        ...
        for ep in range(1, num_episodes + 1):
            ...
            if ep % print_every == 0:
                print(
                    f"[Ep {ep:4d}] "
                    f"avg_metric={avg_metric:.3f}  min_metric={min_metric:.3f}  "
                    f"avg_reward={avg_reward:.3f}  best_metric={self.best_metric:.3f}  "
                    f"loss={loss.item():.3f}"
                )
                cmd = f"cp fourier_crosstalk/fourier_crosstalk_agent.bin logs/fourier_crosstalk/ep{ep:03d}.bin"
