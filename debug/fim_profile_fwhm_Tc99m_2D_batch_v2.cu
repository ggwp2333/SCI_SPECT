...
static constexpr int CUBE_WORKERS = 16;   // was 8; tune 8/16/32
static constexpr int SAMPLE_LANES = 32; 
    ...
    unsigned long long seed_base)
{
    ...
    // block-level tiling
    int v_global = blockIdx.x;     // which tile of voxels
    int ang      = blockIdx.y;     // angle index

    int sample_id = threadIdx.x;   // 0 .. VOXELS_PER_BLOCK-1
    int worker    = threadIdx.y;   // 0 .. CUBE_WORKERS-1 (which subset of cubes)

    if (v_global >= v_count) return;

    curandState state;
    unsigned long long seq = (((unsigned long long)v_global * NANG + ang) * CUBE_WORKERS + worker) * SAMPLE_LANES + sample_id;
    ...
    for(int j = worker; j < Ncubes; j += CUBE_WORKERS){
        ...
        for(int n = sample_id; n < NSAMPLES; n += SAMPLE_LANES){
        ...
        for (int offset = 16; offset > 0; offset >>= 1)
            prob_sum += __shfl_down_sync(0xFFFFFFFF, prob_sum, offset);

        // lane 0 writes the final sum — plain store, no atomic
        if (sample_id == 0) {
            size_t row = (size_t)ang * Ncubes + j;
            size_t col = (size_t)v_global;
                   ...
                   std::vector<double>& F)
{
    ...
    dim3 blk(SAMPLE_LANES, CUBE_WORKERS); 
    dim3 grd(ws.v_count, NANG);
